class BookingPreviewsController < ApplicationController
  ...
  def create
    ...
    @booking_preview = BookingPreview.new(room: room, room_name: room.name, amount: room.price, state: 'pending', user: current_user, start_date: params[:start_date][0..9], end_date: params[:end_date].to_date - 1)
  ...
  def booking_params
    params.require(:booking_previews).permit(:start_date, :end_date, :room_id, :user_id)
  end
